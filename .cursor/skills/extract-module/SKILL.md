...
金标准参考：`packages/modules/notes/`

## 何时使用

- 将遗留路由迁入 `packages/modules/<id>/`
- 将域逻辑收拢到模块 service
- 拆分后从中央注册表移除引用
...
3. 若涉及 Prisma model，在 `apps/server/prisma/models/<id>.prisma` 添加，并在 `scripts/lib/module-dependency-rules.ts` 的 `SCHEMA_FILE_OWNER` 登记归属
4. 紧耦合子域可同包：参考 `packages/be-water/MODULE.md`

## Server checklist

1. 创建 `packages/modules/<id>/MODULE.md`
2. 实现 `ServerAppModule`：`shared.permissions`（如有 PBAC）、`server.registerRoutes`
3. 路由迁至 `packages/modules/<id>/server/`，使用 `defineRoute` + `app.requirePermission`
...
1. 在 `packages/modules/<id>/client/` 实现 `module.tsx`
