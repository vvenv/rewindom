...
- 路由组装：`src/kernel/`、`src/app-shell-routes.tsx`、`src/render-app-routes.tsx`
- 组装层导航契约：`src/app-nav.ts`

基础设施模块前端在 `@be-water/modules/<id>/client`；业务模块在其自有 scope 的包中（当前无）。通用基建在 `@be-water/client-kit`、`@be-water/ui`，产品壳层在 `src/shell/`。
