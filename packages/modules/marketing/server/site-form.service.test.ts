...
vi.mock("./site.service.js", () => ({ getPublishedPublicPage: vi.fn() }));
...
describe("字段表以已发布正文为准，不信客户端", () => {
  it("多送的字段不入库", async () => {
    ...
    const { data } = vi.mocked(prisma.marketingFormSubmission.create).mock
  ...
  it("下拉送了列表外的值 → 逐字段驳回", async () => {
    ...
    expect(prisma.marketingFormSubmission.create).not.toHaveBeenCalled();
  });
