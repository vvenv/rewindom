...
installTestPermissionCatalog([
  { key: "notes.read", label: "查看笔记", group: "示例模块" },
  { key: "notes.write", label: "创建/编辑笔记", group: "示例模块" },
]);
...
describe("Note Routes 权限控制", () => {
  ...
  beforeAll(async () => {
    ...
    await grantPermission(app, reader.id, "notes.read");
  });
  ...
  describe("notes.write", () => {
    it("只有 notes.read 的用户写入返回 403", async () => {
      const response = await app.inject({
        method: "POST",
        url: "/api/notes/",
        headers: authHeaders(reader),
    ...
    it("只有 notes.read 的用户删除返回 403", async () => {
      const response = await app.inject({
        method: "DELETE",
