...
租户用户 CRUD、密码重置、角色分配。工作台登录会话（HttpOnly cookie）与
双因素认证（TOTP）在 `server-kernel` 认证路由；本模块提供用户管理 UI、账户菜单
内的改密 / 2FA 入口，以及增量功能契约（见 `features/`）。

## 租户侧入口

| 路径 | 页面 | 所需权限 |
| --- | --- | --- |
| `/app/users` | 用户列表、增删改、重置密码、分配角色 | `users.read`（写操作另需 `users.write`；分配角色需 `roles.assign`） |

导航项在 `client/shell/user-nav.ts`，与 `rbac` 的 `/app/roles` 同属「系统管理」分组
（`placement: "end"`，侧栏沉底）。
角色**本身**的增删改在 `/app/roles`（`rbac` 模块），这里只做「把已有角色分配给某个用户」。
