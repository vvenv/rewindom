 ...
 *
 * 版式是一张居中的认证卡（抬头 → 第三方登录 → 表单 → 另一张页面的入口），结构与编辑器
 * 预览 `MemberAuthFormSection.tsx` 同构，`MemberAuthFormSection.test.tsx` 守着这条对齐。
 *
...
/**
 * 第三方登录那一块：按钮行 + 「或者用邮箱」的分隔线，排在表单**上面**。
 *
 * 一家都没配就整块不画——留一条孤零零的分隔线，页面上就是一道没有来由的横线。
 */
function oauthHtml(
  ...
  const buttons = providers
    ...
    .join("");
  return `<div class="member-auth-oauth-row">${buttons}</div><div class="member-auth-divider"><span>${escapeHtml(divider)}</span></div>`;
}

/** 认证卡的抬头：不走通用 `sectionHeading`——卡里的标题该居中、该比区块标题小一号。 */
function headHtml(heading: string, subheading: string): string {
  if (!heading && !subheading) return "";
  return `<div class="member-auth-head">${heading ? `<h2>${escapeHtml(heading)}</h2>` : ""}${
...
function renderAuthForm(
  ...
  const oauth = settingBool(s, "show_oauth")
    ...
    : "";
  const cardClass = settingBool(s, "show_card")
    ? "member-auth-card"
    : "member-auth-card is-plain";

  return `<div class="${cardClass}">${headHtml(
    settingText(s, "heading"),
    settingText(s, "subheading"),
  )}${oauth}<form class="member-auth" method="post" action="${escapeHtml(auth.action)}" data-member-auth="${mode}">
  ${error}
