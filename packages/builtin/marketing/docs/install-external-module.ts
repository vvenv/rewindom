...
\`\`\`json
{
  ...
  "dependencies": {
    "@be-water/module-sdk": "workspace:*",
    "@be-water/ui": "workspace:*"
  },
  "peerDependencies": {
    "fastify": "^5.11.2",
    "react": "^19.0.0",
...
> module-sdk 拆分为三个子入口，避免 server-kernel 的 fastify/prisma 类型泄漏到 client 编译上下文。

## 接入流程

\`\`\`bash
# 1. 把模块包放到 packages/external-modules/<moduleId>/
...
1. **发现** \`packages/external-modules/*\` 下的模块包
...
export const myModule: ServerAppModule = {
  id: "my-module",
  onBoot: async () => {
    registerTenantGatedRoutes("/api/my-module", (app) => {
      app.get("/", async (req) => {
        return { data: { items: [] } };
      });
    });
  ...
  i18n: {
    namespaces: ["my-module"],
    bundles: serverI18nBundles,
  } satisfies ServerI18nBundle,
};
\`\`\`
...
export const myModule: ClientAppModule = {
  id: "my-module",
  renderRoutes: () => <MyRoutes />,
  nav: {
    sections: [{ label: "My Module", path: "/app/my-module" }],
  },
};
\`\`\`
...
> 迁移统一由 \`apps/server\` 管理——一个数据库只有一条迁移历史。外部模块只声明 schema，迁移由消费方生成。
