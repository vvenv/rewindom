...
`ns:key`：marketing 自带首页槽位（`home`，空白、不预填段）和 404；文档版式由
site-docs 登记，会员页由 site-member 登记。文案在创建时展开成整张 `__i18n` 表，
公开面再按 URL 语言压扁；套完随便改。预设 `text` 与对应 setting 的 `default`
应是同一条 key。
...
预设链接组。产品首页由模块贡献版式（如 `events.home`）；默认租户产品站另走
`default-product-site-content`。文档版式、会员页等模板页也会快照成可编辑记录，
内容仍是各模块的内置预设。关于我们、联系、定价等普通页面由租户自己在 CMS 里新建，
...
选另一张页 → `home_path` 随保存改写，SSR **不 30x 改写 `/`**——站点根 URL 不变，canonical
...
只读用户（`site.read` 无 `site.write`）也能开 Sheet：字段禁用、没有保存按钮。
...
| **默认租户产品站** | `server/default-product-site-content.ts` | Rewindom 终稿：中英双语首页（hero + 多段 prose + Yestino 案例 + band）；文案来自 `client/locales` 的 `site` / `hero` / `features` / `landing` / `seo` |
