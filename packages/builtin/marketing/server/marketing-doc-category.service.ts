...
function toCategory(record: MarketingDocCategoryRecord): MarketingDocCategory {
  return {
    id: record.id,
...
export async function listDocCategories(
  ...
  const records = await prisma.marketingDocCategory.findMany({
    where: withTenantScope(tenant_id),
...
export async function getDocCategory(
  ...
  const record = await prisma.marketingDocCategory.findFirst({
...
export async function updateDocCategory(
  ...
  const existing = await prisma.marketingDocCategory.findFirst({
    where: withTenantScope(tenant_id, { id: category_id }),
  });
  if (!existing) throw new NotFoundError("site.doc_category_not_found");
  const updated = await prisma.marketingDocCategory.update({
...
export async function deleteDocCategory(
  ...
  if (!existing) throw new NotFoundError("site.doc_category_not_found");
  const inUse = await prisma.marketingDoc.findFirst({
    where: withTenantScope(tenant_id, {
      OR: [
  ...
  await prisma.marketingDocCategory.delete({
    where: { id: category_id, tenant_id },
  });
}
...
export async function resolveDocCategoryKey(
  ...
  const found = await prisma.marketingDocCategory.findFirst({
...
export async function seedDefaultDocCategories(
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    if (!label) continue;
    await prisma.marketingDocCategory.upsert({
