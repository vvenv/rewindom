 ...
 * 文档分类：稳定 key + 多语言显示名。
 *
 * 文档各语言版本共用同一个 `category` key（存在 `MarketingDoc.category`），
 * 显示名存在 `MarketingDocCategory.label`（纯字符串或 `{ __i18n }`），与站名 /
...
export interface MarketingDocCategory {
  id: string;
  tenant_id: string;
  key: string;
...
export interface CreateMarketingDocCategoryBody {
  key: string;
  label: string | LocalizedText;
  sort_order?: number;
}

export interface UpdateMarketingDocCategoryBody {
  label?: string | LocalizedText;
  sort_order?: number;
}
