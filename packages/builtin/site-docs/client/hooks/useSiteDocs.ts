...
function docFiltersEqual(
  left: SiteDocListQuery | undefined,
  right: SiteDocListQuery,
): boolean {
...
/** 文档库列表页：服务端筛选 / 排序 / 分页。 */
export function useSiteDocs(query: SiteDocListQuery) {
  return useQuery({
...
export function useCreateSiteDocCategory() {
  ...
  return useMutation({
    mutationFn: (body: CreateSiteDocCategoryBody) =>
...
export function useReorderSiteDocCategories() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: ReorderSiteDocCategoriesBody) =>
...
export function useCreateSiteDoc() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: CreateSiteDocBody) => createSiteDoc(body),
    onSuccess: () =>
