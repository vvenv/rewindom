 ...
 * 路由，渲染复用站点 chrome + `.prose` 排版（见 `site-doc.service.ts`）。
 *
...
import { resolveLocaleSegment } from "../../marketing/shared/site-locale.js";

import type { SiteDocCategory } from "./site-doc-category.js";

export type SiteDocStatus = "draft" | "published";
...
export interface SiteDocListItem {
  id: string;
  slug: string;
  locale: AppLocale;
  title: string;
...
export interface SiteDocListQuery {
  q?: string;
  category?: string;
...
export interface SiteDocListResult {
...
export interface CreateSiteDocBody {
  slug: string;
  title: string;
  description?: string;
  body_md?: string;
...
export interface UpdateSiteDocBody {
  slug?: string;
  title?: string;
  description?: string;
  body_md?: string;
 ...
 */
export interface DuplicateSiteDocBody {
  title: string;
  locale?: string;
}
 ...
 * 公开面的文档目录条目（不含正文）。
 *
 * `site-docs.list` / `site-docs.nav` 只需要这些字段——列表页不该把每篇的 `body_md` 都拖进内存，
...
/** 公开面的单篇文档（详情模板页渲染 `site-docs.article` / `site-docs.toc` 用）。 */
 ...
 * 编辑器预览的 `registerEditorContextProvider`、公开 SSR 的 section context。少了任何
 ...
 * 否则 `site-docs.toc` 生成的链接在其中一端点不动。刻意**不去重**：同名标题只锚到第一处，
 * 比两端各编一套 `-2` 后缀然后对不上要好。
 */
...
/**
 * 从 markdown 正文抽章节标题（`site-docs.toc` 的数据源）。
 *
