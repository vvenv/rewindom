...
import { withSiteLocale } from "../../marketing/shared/site-locale.js";
import { registerChromeBlockHtml } from "../../marketing/shared/sections/_common/chrome-html.js";
...
import type { ChromeRenderInput } from "../../marketing/shared/sections/_common/chrome-html.js";
import type { SiteBlock } from "../../marketing/shared/section-schema.js";
...
function renderSearchHtml(
  ...
  const label = escapeHtml(docMessages(input.ctx.locale).search);
  const action = withSiteLocale(
    DOCS_INDEX_PATH,
    input.ctx.locale,
    input.ctx.defaultLocale,
  );
