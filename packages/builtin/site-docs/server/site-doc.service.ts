 ...
 * 不进 section / block 体系。**版式**归两张文档模板页（`docs_index` / `docs_article`），
 * 由 `site-docs.*` 段消费这里吐出来的数据，见 `docs.ssr.ts`。
 *
...
import {
  ...
  parseUpdateDocBody,
  validateDocSlug,
  type CreateSiteDocBody,
  type DuplicateSiteDocBody,
  type SiteDoc,
  type SiteDocListItem,
  type SiteDocListQuery,
  type SiteDocListResult,
  type SiteDocStatus,
  type PublicDocDetail,
  type PublicDocSummary,
  type UpdateSiteDocBody,
} from "../shared/site-doc.js";
import { type PageLocaleAlternate } from "../../marketing/shared/site-cms.js";
import { siteLocaleOrder, withSiteLocale } from "../../marketing/shared/site-locale.js";
...
const DOC_LIST_SORT_COLUMN: Record<string, keyof SiteDocRecord> = {
  title: "title_draft",
  slug: "slug",
...
function asStatus(value: string): SiteDocStatus {
  return value === "published" ? "published" : "draft";
}

/** 草稿是否与线上一致（逐字段比对）。未发布的文档恒为 false（线上列是建页初值）。 */
function isContentDirty(record: SiteDocRecord): boolean {
...
export function toSiteDoc(record: SiteDocRecord): SiteDoc {
  return {
    id: record.id,
...
): Prisma.SiteDocWhereInput {
  const and: Prisma.SiteDocWhereInput[] = [];
...
function resolveDocListOrderBy(
...
): Prisma.SiteDocOrderByWithRelationInput[] {
  ...
  return [{ [column]: dir } as Prisma.SiteDocOrderByWithRelationInput];
}
...
export async function listDocs(
  ...
  const [facetRows, total_all, matched] = await Promise.all([
    prisma.siteDoc.findMany({
      where: withTenantScope(tenant_id),
      select: { category_draft: true, locale: true },
    }),
    prisma.siteDoc.count({ where: withTenantScope(tenant_id) }),
    dirtyOnly
      ? prisma.siteDoc.findMany({ where, orderBy })
      : Promise.all([
          prisma.siteDoc.findMany({
            where,
            orderBy,
  ...
  if (dirtyOnly) {
    const dirty = (matched as SiteDocRecord[])
  ...
  const [records, total] = matched as [SiteDocRecord[], number];
  const page_count = Math.max(1, Math.ceil(total / page_size) || 1);
  return {
...
export async function createDoc(
  ...
  const category = parsed.category
    ...
    : "";
  try {
    const created = await prisma.siteDoc.create({
      data: {
        tenant_id,
        slug: parsed.slug,
        locale,
...
export async function updateDoc(
  ...
  const existing = await prisma.siteDoc.findFirst({
    where: withTenantScope(tenant_id, { id: doc_id }),
  });
  if (!existing) {
    throw new NotFoundError("site.doc_not_found");
  ...
  const data: Prisma.SiteDocUpdateInput = {};
  ...
  try {
    const updated = await prisma.siteDoc.update({
      where: { id: doc_id, tenant_id },
      data,
    });
    return toSiteDoc(updated);
  } catch (err) {
    if (
      err &&
      typeof err === "object" &&
...
export async function deleteDoc(
  ...
  await prisma.siteDoc.delete({ where: { id: doc_id, tenant_id } });
}
...
export async function duplicateDoc(
  ...
  const source = await prisma.siteDoc.findFirst({
    where: withTenantScope(tenant_id, { id: doc_id }),
  });
  if (!source) {
    throw new NotFoundError("site.doc_not_found");
  ...
  const clash = await prisma.siteDoc.findFirst({
 ...
 * 摘要投影。
 *
 * 参数按**结构**收而不是收整条 `SiteDocRecord`：目录查询已经用 `select` 只取
...
export async function hasPublishedDocs(tenant_id: string): Promise<boolean> {
  const found = await prisma.siteDoc.findFirst({
    where: withTenantScope(tenant_id, { status: "published" }),
    select: { id: true },
  });
  return found !== null;
