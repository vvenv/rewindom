...
/** 会触发拉 feed 的段：Rising / Now / 存量 feed。 */
export const EVENTS_FEED_CONTEXT_TYPES = EVENTS_FEED_SECTION_TYPES;

/** 「全部主题」在 setting 里用空串表示，与 marketing 的 select 取值口径一致。 */
export const EVENTS_FEED_TOPIC_ALL = "";

/** 与 setting `limit` 的 default / max 对齐；allocate 与取数都读这里。 */
export const EVENTS_RISING_LIMIT_DEFAULT = 4;
export const EVENTS_NOW_LIMIT_DEFAULT = 8;
export const EVENTS_FEED_LIMIT_DEFAULT = 6;
export const EVENTS_RISING_LIMIT_MAX = 12;
export const EVENTS_NOW_LIMIT_MAX = 12;
...
export function eventFeedLimitDefault(type: string): number {
  if (type === EVENTS_NOW_SECTION_TYPE) return EVENTS_NOW_LIMIT_DEFAULT;
  if (type === EVENTS_FEED_SECTION_TYPE) return EVENTS_FEED_LIMIT_DEFAULT;
...
export const eventsFeedSection: SectionDefinition = {
  ...
  settings: [
    ...
    ...eventFeedContentSettings(EVENTS_FEED_LIMIT_DEFAULT),
  ],
};
