...
function view(
  ...
  return {
    href: "/events/entity",
    items: [
      { href: "/events/entity/openai-abc123", name: "OpenAI", event_count: 8 },
      { href: "/events/entity/google-def456", name: "Google", event_count: 5 },
      { href: "/events/entity/chatgpt-ghi789", name: "ChatGPT", event_count: 1 },
    ],
    ...overrides,
  };
}
...
describe("renderEventsEntityStripHtml", () => {
  it("links every shown entity — this strip exists so the home page stops being a dead end", () => {
    ...
    expect(html).toContain('href="/events/entity/google-def456"');
    expect(html).toContain('href="/events/entity/chatgpt-ghi789"');
  });
  ...
  it("points See all at the entity hub", () => {
    const html = render(view());
    expect(html).toContain('href="/events/entity"');
