...
const sig = (title: string, excerpt = ""): ExtractableSignal => ({
  title,
  excerpt,
});
...
describe("金额", () => {
  ...
  it.each([
    ...
    [
      "SEC Charges Toms River Trio in Alleged $47 Million Fraud",
      "$47 Million",
      47e6,
    ],
    [
      "FTC Stops Scheme that Scammed Consumers Out of Nearly $200 Million",
      "$200 Million",
      200e6,
    ],
    [
      "Startup closes a round at a $1,200,000 valuation",
      "$1,200,000",
      1_200_000,
    ],
  ])("%s → %s", (title, text, usd) => {
   ...
   */
  it("非美元保留原串，但不折算", () => {
    const facts = extractEventFacts("funding", [
      sig("Acme raises €2 million in a seed round"),
    ]);
  ...
  it("标题优先于摘录", () => {
    const facts = extractEventFacts("acquisition", [
      sig(
        "Acme acquires Beta for $300 million",
        "Beta had raised $45 million to date",
      ),
    ]);
  ...
  it("发版与故障不抽金额", () => {
    expect(
      extractEventFacts("release", [sig("Pro plan now $20/mo")]).amount_text,
    ).toBeNull();
    expect(
      extractEventFacts("outage", [sig("Refunds up to $50")]).amount_text,
    ).toBeNull();
  });
});
...
describe("版本号", () => {
  ...
  it("价格不是版本号", () => {
    expect(
      extractEventFacts("release", [
        sig(
          "Fairphone's latest phone goes on sale",
          "It costs $649.99 in the US",
        ),
      ]).version,
    ).toBeNull();
  });
  ...
  it("单段数字不算版本号", () => {
    expect(
      extractEventFacts("release", [sig("Windows 11 now available")]).version,
    ).toBeNull();
    expect(
      extractEventFacts("release", [sig("GPT 5 launches today")]).version,
    ).toBeNull();
  });
});
...
describe("边界", () => {
  it("判不出类型时什么都不抽", () => {
    const facts = extractEventFacts(null, [
      sig("Acme acquires Beta for $300 million"),
    ]);
