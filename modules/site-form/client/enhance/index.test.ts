...
import { enhanceSite } from "./index.js";

describe("enhanceSite", () => {
  beforeEach(() => {
    ...
    `;
    enhanceSite({ locale: "en", pagePath: "/contact" });
  });
  ...
  it("提交成功后换成 success", async () => {
    ...
    // 页面路径来自 boot 时的上下文快照，不是提交时再去翻 DOM
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/public/site-form/submit",
      expect.objectContaining({
        body: expect.stringContaining('"path":"/contact"'),
      }),
    );
  });
});
