 ...
 * 公开表单的提交与查看。
 *
 * 提交是整套官网里唯一一条**匿名写库**的路径，所以校验口径写死在这里、不信任何客户端
...
import { SITE_FORM_SECTION_TYPE } from "../shared/sections/form/definition.js";
import {
  resolveFormFields,
  validateFormValues,
  type FormEntry,
...
export async function submitSiteForm(input: {
  ...
  if (!section || section.type !== SITE_FORM_SECTION_TYPE) {
...
export async function deleteFormSubmission(
  ...
  const result = await prisma.siteFormSubmission.deleteMany({
